...
export class UserService extends BaseService {
  ...
  sql_insert: string = `
INSERT INTO User(id, name, date_of_birth, address, phone_number, version, created_on, modified_on, is_active)
...
`;
  ...
  sql_delete: string = ` DELETE FROM public.User
   WHERE id = $1
   RETURNING *; `;

  public async select(_req: UserWrapper): Promise<Array<UserWrapper>> {
    ...
    try {
      ...
      if (rows.length > 0) {
        _.forEach(rows, (v) => {
          ...
          temp.phone_number =
            v.phone_number != null ? parseInt(v.phone_number) : null;
